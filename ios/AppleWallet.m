...
- (NSDictionary *)constantsToExport {
    ...
    return @{
        ...
        @"PaymentButtonBuyWidth": @(CGRectGetWidth(paymentButton.frame) * 1.5), // payment button default width is not suitable for our use case
        @"PaymentButtonPlainWidth": @(CGRectGetWidth(paymentButton.frame) / 1.75), // payment button default width is not suitable for our use case
        @"PaymentButtonHeight": @(CGRectGetHeight(addPassButton.frame)), // this is not a mistake, we want payment button height to be of ths same as add pass
        @"PaymentButtonType": @{
...
- (void)cleanup {
    self.addPaymentPassRequestCompletionHandler = nil;
    self.cardholderName = nil;
    self.localizedDescription = nil;
    self.paymentNetwork = nil;
    self.primaryAccountIdentifier = nil;
    self.primaryAccountSuffix = nil;
    self.leafCertificate = nil;
    self.subCACertificate = nil;
    self.nonce = nil;
    self.nonceSignature = nil;
}

RCT_REMAP_METHOD(dismissAddPaymentPassViewController,
                 resolve:(RCTPromiseResolveBlock)resolve
                 reject:(RCTPromiseRejectBlock)reject) {
        ...
        if (window) {
            ...
            if (rootViewController) {
                ...
                if (presentedViewController) {
                    [rootViewController dismissViewControllerAnimated:presentedViewController completion:^{
                        [self cleanup];
                    ...
                    [self cleanup];
                    reject(@"AddPaymentPassViewController dismissal error", @"AddPaymentPassViewController was not dismissed", nil);
                    return;
                }
            }
            else {
